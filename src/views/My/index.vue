<template>
  ...
  <div class="my-contain">
    <div class="userInfo" v-if="userlist.length !== 0">
      <div class="box">
        <van-image
          round
          class="avatar"
        ...
        />
        <span class="text">{{ userlist.username }}</span>
      </div>
      <van-cell icon="location-o" title="我的账户" is-link>
        <van-button
          round
          class="user-btn"
          size="mini"
          :to="{ path: '/account', query: { userId: userlist.userId } }"
          >立即充值</van-button
        >
      </van-cell>
      <van-cell icon="like-o" title="我的阅历" is-link />
      <van-cell icon="gem-o" title="充值记录" is-link />
      <van-cell icon="balance-o" title="购买记录" is-link />
      <van-cell icon="search" title="浏览历史" is-link />
      <van-cell icon="points" title="我的阅读基因" is-link />
      <van-cell icon="award-o" title="设置" is-link />
      <van-cell class="goout" title="退出登录" @click="onLogout" />
    </div>
